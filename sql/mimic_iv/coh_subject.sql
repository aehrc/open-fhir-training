...
CREATE OR REPLACE VIEW mimic_iv_views.coh_subject AS
SELECT
    subject_id,
    gender,
    race_category
FROM mimic_iv_views.st_subject AS sbj
WHERE EXISTS(
    SELECT 1
    FROM mimic_iv_views.st_reading_o2_flow AS rof
    WHERE sbj.subject_id = rof.subject_id
)
AND EXISTS(
    SELECT 1
    FROM mimic_iv_views.st_reading_spo2 AS rspo2
    WHERE sbj.subject_id = rspo2.subject_id
)
AND EXISTS(
    SELECT 1
    FROM mimic_iv_views.st_reading_so2 AS rso2
